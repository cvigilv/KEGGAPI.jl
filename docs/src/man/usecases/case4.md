...
Retrieve information on the associated drugs by passing
`drugs.columns.target_id` to `kegg_get`:

```@example case4
drug_info = KEGGAPI.kegg_get(drugs.columns.target_id)
