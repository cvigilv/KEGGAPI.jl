...
Retrieve information on the associated drugs by passing the second field from
each `drugs` row to `kegg_get`:

```@example case4
drug_info = KEGGAPI.kegg_get([row[2] for row in drugs])
